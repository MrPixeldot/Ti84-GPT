  ...
  constexpr auto TIP = 21;
  constexpr auto RING = 22;
...
int makeRequest(String url, char *result, int resultLen, size_t *len)
{
    ...
    client.setInsecure(); // allow self-signed cert
#else
    WiFiClient client;
#endif
    HTTPClient http;
    ...
    {
        if (totalRead >= resultLen - 1) break;
        result[totalRead++] = httpStream->read();
    }
    result[totalRead] = '\0';
    *len = totalRead;
  ...
  void gpt() {
      ...
      // Start the conversation string
      fullResponse = "User: " + String(prompt) + " | Gemini 2.5: ";

      // Replace the endpoint to Gemini 2.5
      auto url = String(SERVER) + String("/google/ask?question=") + urlEncode(String(prompt));

      size_t realsize = 0;
      if (makeRequest(url, response, MAXHTTPRESPONSELEN, &realsize)) {
          setError("error making request to Gemini 2.5");
          return;
      }

      fullResponse += String(response);
      Serial.print("Full Gemini 2.5 response: ");
      Serial.println(fullResponse);

      PAGE_PAGE = 0;  // Reset pagination
      sendPage();
  }
